...
public class ScheduleServiceImpl implements ScheduleService, ScheduleServiceForHallBl,ScheduleServiceForBl {
    ...
    public ResponseVO addSchedule(ScheduleForm scheduleForm) {
        try {
            ...
            e.printStackTrace();
            return ResponseVO.buildFailure("排片失败");
        }
    }
    ...
    ResponseVO preCheck(ScheduleForm scheduleForm){
        try {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
            // 检查排片时间是否早于当前时间
            System.out.println(scheduleForm.getStartTime());
    ...
    @Override
    public List<ScheduleItem> getScheduleByHallID(int hall_id){
        try{
            List<ScheduleItem> scheduleItemList = scheduleMapper.selectScheduleByHallId(hall_id);
